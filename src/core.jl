...
for R=(:R1, :R2, :R3, :R4)
    ...
    function $(R)(g::TimeGrid{<:Real,3}, ws, ls)
        ls_protected = []
        # this was needed as ls is of a tuple of LineshapesLUT which should be protected from broadcasting
        for l in ls
            push!(ls_protected, (l,))
        end
        return $(R)(grid(g)..., ws..., ls_protected...)
    end
    # here the problem is that some of the things in a do need not to be broadcasted ...
